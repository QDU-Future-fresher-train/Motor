...
enum class ControlMode : uint8_t {
    MODE_TORQUE = 0,     // 力矩控制（N·m） ← 推荐闭环使用
    MODE_CURRENT         // 电流控制（归一化 -1.0 ~ 1.0 或直接电压比例）
};
...
public:
    /* 构造时必须传入的参数 */
    struct Param {
        bool        reverse;          // 是否反转（true = 方向相反）
        uint16_t    feedback_id;      // 电机反馈 CAN ID（0x205~0x20B）
        const char* can_bus_name;     // CAN总线名称，例如 "can1"、"can2"
    };
...
public:
    ...
    void Enable()           { /* GM6020上电即使能，此处为空 */ }
    void Disable()          { CurrentControl(0.0f); }
    void Relax()            { CurrentControl(0.0f); }
